...
const CACHE_VERSION = 'v137'; // 07-09-2026 : CORRECTIF CRITIQUE — le capital de l'entreprise revenait à 0 après un import, dès l'actualisation/la reconnexion/la synchronisation suivante (voir index.html v75). Cause exacte : un import restaure des produits Stock Entreprise et/ou des réductions manuelles de capital supprimés/vidés AVANT lui (ex: "Vider Tout") ; leur tombstone, encore présent côté cloud tant que l'envoi post-import n'a pas abouti, était réappliqué à la première fusion (union inconditionnelle des tombstones) et effaçait de nouveau ce que l'import venait de restaurer. Corrigé en horodatant chaque import (D._importedAt) et en neutralisant, dans mergeAppData(), uniquement les tombstones obsolètes du côté qui n'a pas encore vu cet import, et uniquement pour les identifiants réellement présents dans les données importées. Testé par simulation Node.js réelle (jsdom, réseau Supabase simulé, y compris la course import→réconciliation avant confirmation de l'envoi cloud) : capital exact après Export→Import→Actualisation/Reconnexion/Synchronisation, y compris pour la réduction manuelle du capital ; non-régression complète v69-v74 (dont "Vider Tout"/"Vider Perso", toujours corrects). Aucun autre calcul, aucun renommage, aucune suppression de donnée non prévue.

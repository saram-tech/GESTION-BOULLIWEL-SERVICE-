...
const CACHE_VERSION = 'v135'; // 07-09-2026 : CORRECTIF CRITIQUE — suppression Stock Entreprise non persistante dans Supabase (voir index.html v73). delStock() posait un tombstone local puis un simple upsert (save()), qui n'efface jamais une ligne absente du tableau envoyé : la ligne restait en base et pouvait réapparaître à tout rechargement/reconnexion/synchronisation. Corrigé en déclenchant, comme pour "Vider le Stock Entreprise", une vraie suppression serveur (DELETE ciblé par id, jamais par nom) via supprimerLotTableSurServeur(). La protection du capital pour reste=0 (v72) reste intacte. Testé par simulation Node.js réelle (jsdom, vrai réseau Supabase simulé avec table "stock" persistante, DELETE vérifié, absence confirmée après reload et sur un second appareil). Aucun autre calcul, aucun renommage, aucune suppression de donnée non prévue.

...
const CACHE_VERSION = 'v136'; // 07-09-2026 : CORRECTIF CRITIQUE — "Vider" le Stock Perso ne remettait pas à zéro "Reste que l'entreprise me doit" (voir index.html v74). viderPerso() vidait D.perso sans jamais réinitialiser D.entDoitBase (la référence de la dernière clôture) : l'ancienne référence restait affichée dans Stock Perso ET dans le Tableau de bord (même formule). Corrigé en remettant D.entDoitBase à 0, horodaté (_entDoitBaseAt), exactement comme "Tout" (viderTout — déjà vérifié correct, non modifié). La donnée source Supabase (parametres.ent_doit_stock_perso) se remet à 0 via le mécanisme existant inchangé. Testé par simulation Node.js réelle (jsdom, réseau Supabase simulé, y compris la table "parametres" avec sémantique d'upsert par colonne fidèle à Supabase) : 97/97 tests OK (nouveau correctif + non-régression complète v69-v73). Aucun autre calcul, aucun renommage, aucune suppression de donnée non prévue.

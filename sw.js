...
const CACHE_VERSION = 'v131'; // 06-09-2026 : CORRECTIF CRITIQUE — synchronisation Stock Perso multi-appareils (voir index.html v69). enrPerso() ne posait jamais _localTouchedAt sur les fiches créées/mises à jour, contrairement à toutes les autres tables fusionnables. mergeAppData() ne pouvait donc pas déterminer quelle version d'une même fiche Stock Perso (modifiée séparément sur deux appareils) était réellement la plus récente, et pouvait écraser une modification récente (montant dû, prix) par une plus ancienne — seule la quantité était protégée. Corrigé : _localTouchedAt posé partout dans enrPerso(), et 'perso' ajouté à la liste des tables bénéficiant de la résolution fine par horodatage (même mécanisme déjà validé pour Dettes/Paiements/Dépenses/Commandes/Patrimoine). Testé par simulation Node.js réelle (nouveau produit conservé, conflit multi-appareils résolu par la modif la plus récente, tombstones et anti-recul de quantité toujours fonctionnels). Aucun calcul ni autre fonctionnalité modifiés. Voir aussi v130/v129/v128.
